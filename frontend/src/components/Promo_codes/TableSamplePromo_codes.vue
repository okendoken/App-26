...
promo_codesStore.fetch()

/* Delete item */
const deleteItem = () => {
  promo_codesStore.deleteItem(currentId.value)
}

const items = computed(() => promo_codesStore.data)
...
<template>
  ...
  <table>
    <thead>
      ...
      <th
        :class="['sortable uppercase text-sm font-normal text-pavitra-600', props.sortTitle === 'code' && props.sortDirection]"
        @click="sort('code')"
      >Code</th>
      ...
      <th
        :class="['sortable uppercase text-sm font-normal text-pavitra-600', props.sortTitle === 'products' && props.sortDirection]"
        @click="sort('products')"
      >Products</th>
    ...
    <tbody>
      <tr
        ...
        :key="client.id"
        @click="$router.push(`/promo_codes/${client.id}/edit`)"
      >
        <TableCheckboxCell
          v-if="checkable"
        ...
        />
              ...
              <td data-label="products">
                  <span
                    v-for="(i, idx) in dataFormatter.productsManyListFormatter(client.products)"
                    :key="idx + client.products"
                    class="block"
                  >
                      {{ i }}
                  </span>
